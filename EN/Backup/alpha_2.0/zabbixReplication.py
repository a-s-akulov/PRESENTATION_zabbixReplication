...
class ZabbixReplication():
    ...
    def __init__(self):
        ...
        # Groups: ZabbixGroup = WorkspaceGroups, TemplateId, ProxyId
        self.GROUPS = {
            '15': ([8], "33112", "0"), # IPSec = ([Роутер], NoTemplate, NoProxy)
            '19': ([6, 19], "33112", "0"), # Камеры = ([Камера, Камера (кабинет)], NoTemplate, NoProxy)
            '16': ([14], "33112", "0"), # WAN1 = (WAN1, NoTemplate, NoProxy)
            '17': ([21], "33112", "0"), # WAN2 = (WAN2, NoTemplate, NoProxy)
            '34': ([20], "33112", "26037"), # KKT = (ККТ, NoTemplate, proxyid='26037')
        }

        # Maintenance groups: City timezone = Zabbix hostGroup (zabbix group in some maintenance group)
        self.maintenanceGroups = {
            3: "43", # timezone 3: Moscow = zabbix hostGroup '43'
    ...
    def auto_replication(self):
        ...
        if len(self.deleteHosts) != 0:
            ...
            for s in self.deleteHosts:
                ...
                self._katprint(f"> Uncorrect host deleted. IP: '{s}', jsonGroup: {host['jsonGroup']}, "\
                    f"maintenanceGroup: '{host['maintenanceGroup']}', Name: '{host['name']}', template: '{host['template']}', "\
                    f"proxy: '{host['proxy']}'")
        ...
        for shopid_ in self.addHosts:
            for ip_ in self.addHosts[shopid_]:
                ...
                self._katprint(f"> Host added. IP: '{ip_}'"\
                    ...
                    f", template: '{host_['host']['template']}'"\
        ...
        if len(self.triggersDependencies) == 0 or len(shopsToRebuildTriggers) == 0:
            self._katprint("> No triggers to rebuild.")
        else:
            self.rebuild_triggers_dependencies(shopsToRebuildTriggers)
            self._katprint("> Triggers dependencies rebuilded succesfully.\n")
    ...
    def get_list_add(self):
        ...
        for s in SQL.sqlHosts:
            ...
            timeDelta = SQL.sqlCities[self.city_from_str(sqlName)]
            sqlMaintenanceGroup = self.get_maintanenceGroup(timeDelta)

            if sqlGroup not in availableGrps:
                continue

            jsonGroup = self.find_group_json(sqlGroup)
            template = self.GROUPS[jsonGroup][1]
    ...
    def get_list_delete(self):
        ...
        for s in JSON.jsonHosts['hosts']:
            ...
            jsonTemplate = jsonData['host']['template']
            ...
            if jsonTemplate != self.GROUPS[jsonGroup][1]:
                self.deleteHosts.append(s)
                continue
