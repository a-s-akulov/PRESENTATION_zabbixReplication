...
class ShowAllTemplates():
    """Print all templates from Zabbix server with templateid for each."""
    def __init__(self):
    ...
    def showList(self):
        """Main fnc - Print all templates from Zabbix server with templateid for each."""
        try:
            query = {
                "jsonrpc": "2.0",
                "method": "user.login",
            ...
            print(f" List of templates at Zabbix server '{self.JSONConfig['server']}':\n")
            ans.sort(key=lambda x: int(x["templateid"]))
            for x in ans:
                print(str(x)[1:-1])
