...
class ShowAllHosts():
    """Print all hosts from Zabbix server with hostid for each."""
    def __init__(self):
    ...
    def showList(self):
        """Main fnc - Print all hosts from Zabbix server with hostid for each."""
        try:
            query = {
                "jsonrpc": "2.0",
                "method": "user.login",
            ...
            print(f" List of hosts at Zabbix server '{self.JSONConfig['server']}':\n")
