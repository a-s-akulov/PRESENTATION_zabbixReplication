    ...
    def get_json_data(self):
        ...
        if "result" in answer:
            for x in answer["result"]:
                ...
                name = x["name"]
                template = x["parentTemplates"][0]["templateid"] if len(x["parentTemplates"]) > 0 else ""
                ...
                if group not in MAIN.GROUPS:
                    if group in MAIN.maintenanceGroups.values():
                ...
                items = self.return_items_by_hostid(hostid)
                for item_ in items:
                    self.jsonHosts['hosts'][ip]['items'][item_['name']] = item_

                triggers = self.return_triggers_by_hostid(hostid)
                for trigger_ in triggers:
                    self.jsonHosts['hosts'][ip]['triggers'][trigger_['description']] = trigger_
